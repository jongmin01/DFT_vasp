...
def find_k_valley_point(kpoints: np.ndarray) -> Optional[int]:
    """
    Find K valley point in k-path

    K valley is at (1/3, 1/3, 0) in fractional coordinates

    Args:
        kpoints: k-point coordinates [nkpts, 3]

    Returns:
        k-point index of K valley (closest to 1/3, 1/3, 0)
    """
    k_target = np.array([1/3, 1/3, 0])

    # Find k-point closest to K valley
    distances = np.linalg.norm(kpoints - k_target, axis=1)
    k_index = np.argmin(distances)

    # Verify it's actually close to K valley
    if distances[k_index] < 0.05:  # Within tolerance
        return k_index

    return None


def find_gamma_point(kpoints: np.ndarray) -> Optional[int]:
    """
    Find Gamma point in k-path

    Gamma point is at (0, 0, 0) in fractional coordinates

    Args:
        kpoints: k-point coordinates [nkpts, 3]

    Returns:
        k-point index of Gamma point
    """
    # Find k-point closest to Gamma
    distances = np.linalg.norm(kpoints, axis=1)
    gamma_index = np.argmin(distances)

    # Verify it's actually Gamma point
    if distances[gamma_index] < 0.01:  # Within tolerance
        return gamma_index

    return None


def find_direct_gap_at_k(energies: np.ndarray, k_index: int, efermi: float) -> Tuple:
    """
    Find direct bandgap at a specific k-point

    Args:
        energies: Band energies [nkpts, nbands]
        k_index: k-point index to analyze
        efermi: Fermi energy

    Returns:
        vbm_band, vbm_energy, cbm_band, cbm_energy, gap
    """
    energies_shifted = energies - efermi
    energies_at_k = energies_shifted[k_index, :]

    # Find VBM at this k-point (highest occupied)
    occupied = energies_at_k < 0
    if not np.any(occupied):
        return None, None, None, None, None

    vbm_band = np.where(occupied)[0][energies_at_k[occupied].argmax()]
    vbm_energy = energies_at_k[vbm_band]

    # Find CBM at this k-point (lowest unoccupied)
    unoccupied = energies_at_k > 0
    if not np.any(unoccupied):
        return vbm_band, vbm_energy, None, None, None

    cbm_band = np.where(unoccupied)[0][energies_at_k[unoccupied].argmin()]
    cbm_energy = energies_at_k[cbm_band]

    gap = cbm_energy - vbm_energy

    return vbm_band, vbm_energy, cbm_band, cbm_energy, gap
    ...
    for idx, (layer_name, color) in enumerate(zip(layer_names, colors)):
        ...
        for iband in range(energies.shape[1]):
            ...
            for i in range(len(kdist) - 1):
                ...
                alpha = 0.2 + 0.8 * (band_proj[i] + band_proj[i+1]) / (2 * projections.max())
...
def analyze_band_character(
    ...
    report = []
    report.append("=" * 70)
    report.append("BAND GAP SUMMARY")
    report.append("=" * 70)
    report.append(f"\nFundamental (Indirect) Gap: {gap:.4f} eV")

    # Add direct gap information upfront
    k_valley_idx = find_k_valley_point(kpoints)
    gamma_idx = find_gamma_point(kpoints)

    if k_valley_idx is not None:
        k_vbm_band, k_vbm_energy, k_cbm_band, k_cbm_energy, k_gap = find_direct_gap_at_k(
            energies, k_valley_idx, efermi
        )
        if k_gap is not None:
            report.append(f"K Valley Direct Gap:       {k_gap:.4f} eV")

    if gamma_idx is not None:
        g_vbm_band, g_vbm_energy, g_cbm_band, g_cbm_energy, g_gap = find_direct_gap_at_k(
            energies, gamma_idx, efermi
        )
        if g_gap is not None:
            report.append(f"Γ Point Direct Gap:        {g_gap:.4f} eV")

    report.append(f"\n" + "=" * 70)
    report.append("FUNDAMENTAL GAP ANALYSIS (INDIRECT)")
    report.append("=" * 70)

    vbm_kcoord = kpoints[vbm_k]
    cbm_kcoord = kpoints[cbm_k]

    report.append(f"\nVBM (Valence Band Maximum):")
    report.append(f"  Energy: {vbm_energy:.4f} eV (relative to E_F)")
    report.append(f"  k-point index: {vbm_k}")
    report.append(f"  k-point coordinates: ({vbm_kcoord[0]:.5f}, {vbm_kcoord[1]:.5f}, {vbm_kcoord[2]:.5f})")
    ...
    report.append(f"  k-point coordinates: ({cbm_kcoord[0]:.5f}, {cbm_kcoord[1]:.5f}, {cbm_kcoord[2]:.5f})")
    ...
    # Analyze K valley direct gap
    report.append(f"\n" + "=" * 70)
    report.append("K VALLEY DIRECT GAP ANALYSIS")
    report.append("=" * 70)

    if k_valley_idx is not None:
        k_coord = kpoints[k_valley_idx]
        report.append(f"\nk-point index: {k_valley_idx}")
        report.append(f"k-point coordinates: ({k_coord[0]:.5f}, {k_coord[1]:.5f}, {k_coord[2]:.5f})")
        report.append(f"Direct bandgap at K: {k_gap:.4f} eV")

        if k_gap is not None:
            report.append(f"\nE_VB(K) - Valence Band at K:")
            report.append(f"  Energy: {k_vbm_energy:.4f} eV (relative to E_F)")
            report.append(f"  Band index: {k_vbm_band}")
            report.append(f"\n  Layer contributions:")

            k_vbm_total = 0
            for layer_name, projections in layer_projections.items():
                k_vbm_proj = projections[k_valley_idx, k_vbm_band]
                k_vbm_total += k_vbm_proj
                report.append(f"    {layer_name:15s}: {k_vbm_proj:8.4f} ({100*k_vbm_proj:.2f}%)")

            report.append(f"    {'Total':15s}: {k_vbm_total:8.4f} ({100*k_vbm_total:.2f}%)")

            report.append(f"\nE_CB(K) - Conduction Band at K:")
            report.append(f"  Energy: {k_cbm_energy:.4f} eV (relative to E_F)")
            report.append(f"  Band index: {k_cbm_band}")
            report.append(f"\n  Layer contributions:")

            k_cbm_total = 0
            for layer_name, projections in layer_projections.items():
                k_cbm_proj = projections[k_valley_idx, k_cbm_band]
                k_cbm_total += k_cbm_proj
                report.append(f"    {layer_name:15s}: {k_cbm_proj:8.4f} ({100*k_cbm_proj:.2f}%)")

            report.append(f"    {'Total':15s}: {k_cbm_total:8.4f} ({100*k_cbm_total:.2f}%)")

            # Find dominant layers at K valley
            k_vbm_dominant = max(layer_projections.items(),
                                key=lambda x: x[1][k_valley_idx, k_vbm_band])
            k_cbm_dominant = max(layer_projections.items(),
                                key=lambda x: x[1][k_valley_idx, k_cbm_band])

            report.append(f"\nDominant layer at E_VB(K): {k_vbm_dominant[0]} "
                         f"({100*k_vbm_dominant[1][k_valley_idx, k_vbm_band]:.1f}%)")
            report.append(f"Dominant layer at E_CB(K): {k_cbm_dominant[0]} "
                         f"({100*k_cbm_dominant[1][k_valley_idx, k_cbm_band]:.1f}%)")
        else:
            report.append("\nWarning: Could not determine direct gap at K valley")
    else:
        report.append("\nWarning: K valley not found in k-path")

    # Analyze Gamma point direct gap
    report.append(f"\n" + "=" * 70)
    report.append("Γ POINT DIRECT GAP ANALYSIS")
    report.append("=" * 70)

    if gamma_idx is not None:
        gamma_coord = kpoints[gamma_idx]
        report.append(f"\nk-point index: {gamma_idx}")
        report.append(f"k-point coordinates: ({gamma_coord[0]:.5f}, {gamma_coord[1]:.5f}, {gamma_coord[2]:.5f})")
        report.append(f"Direct bandgap at Γ: {g_gap:.4f} eV")

        if g_gap is not None:
            report.append(f"\nE_VB(Γ) - Valence Band at Γ:")
            report.append(f"  Energy: {g_vbm_energy:.4f} eV (relative to E_F)")
            report.append(f"  Band index: {g_vbm_band}")
            report.append(f"\n  Layer contributions:")

            g_vbm_total = 0
            for layer_name, projections in layer_projections.items():
                g_vbm_proj = projections[gamma_idx, g_vbm_band]
                g_vbm_total += g_vbm_proj
                report.append(f"    {layer_name:15s}: {g_vbm_proj:8.4f} ({100*g_vbm_proj:.2f}%)")

            report.append(f"    {'Total':15s}: {g_vbm_total:8.4f} ({100*g_vbm_total:.2f}%)")

            report.append(f"\nE_CB(Γ) - Conduction Band at Γ:")
            report.append(f"  Energy: {g_cbm_energy:.4f} eV (relative to E_F)")
            report.append(f"  Band index: {g_cbm_band}")
            report.append(f"\n  Layer contributions:")

            g_cbm_total = 0
            for layer_name, projections in layer_projections.items():
                g_cbm_proj = projections[gamma_idx, g_cbm_band]
                g_cbm_total += g_cbm_proj
                report.append(f"    {layer_name:15s}: {g_cbm_proj:8.4f} ({100*g_cbm_proj:.2f}%)")

            report.append(f"    {'Total':15s}: {g_cbm_total:8.4f} ({100*g_cbm_total:.2f}%)")

            # Find dominant layers at Gamma
            g_vbm_dominant = max(layer_projections.items(),
                                key=lambda x: x[1][gamma_idx, g_vbm_band])
            g_cbm_dominant = max(layer_projections.items(),
                                key=lambda x: x[1][gamma_idx, g_cbm_band])

            report.append(f"\nDominant layer at E_VB(Γ): {g_vbm_dominant[0]} "
                         f"({100*g_vbm_dominant[1][gamma_idx, g_vbm_band]:.1f}%)")
            report.append(f"Dominant layer at E_CB(Γ): {g_cbm_dominant[0]} "
                         f"({100*g_cbm_dominant[1][gamma_idx, g_cbm_band]:.1f}%)")
        else:
            report.append("\nWarning: Could not determine direct gap at Γ point")
    else:
        report.append("\nWarning: Γ point not found in k-path")

    # Add note about K' valley
    if k_valley_idx is not None or gamma_idx is not None:
        report.append(f"\n" + "=" * 70)
        report.append("NOTE")
        report.append("=" * 70)
        report.append(f"\nK' valley not included in current k-path.")
        report.append(f"For complete valley analysis, K' valley calculation is needed.")
...
def main():
    ...
    analyze_band_character(energies, data.kpoints, layer_projections, efermi, args.report)
