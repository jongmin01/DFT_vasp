...
def plot_fatband_multiorbital(
    ...
    # Create custom legend with orbital colors
    from matplotlib.patches import Patch
    legend_elements = [Patch(facecolor=orbital_colors[name], alpha=0.6, label=name)
                      for name in orbital_names]
    legend_elements.append(plt.Line2D([0], [0], color='k', linestyle='--',
                                     linewidth=1.5, label='$E_F$'))
    ax.legend(handles=legend_elements, fontsize=12, loc='upper right',
              framealpha=0.95, ncol=2, edgecolor='black', fancybox=True)
